...
class sendmessage(Callback):
    ...
    def prog(self):  # Show progress
        nb_batches_total = (self.params['nb_epoch'] if not kv - 1 else self.params['epochs']) * self.params[
            'nb_sample'] / self.params['batch_size']
    ...
    def get_fig(self, level='all', metrics=['all']):
        ...
        def batches(color_list='rgbyck' * 10, metrics=['all']):
            if 'all' in metrics:
                ...
                m_available = ([val for val in list(self.logs_batches.keys()) if val in metrics] if [val for val in
                                                                                                     list(
                                                                                                         self.logs_batches.keys())
                                                                                                     if
                                                                                                     val in metrics] else list(
                    self.logs_batches.keys()))
        ...
        def epochs(color_list='rgbyck' * 10, metrics=['all']):
            if 'all' in metrics:
                ...
                m_available = ([val for val in list(self.logs_epochs.keys()) if val in metrics] if [val for val in list(
                    self.logs_epochs.keys()) if val in metrics] else list(self.logs_epochs.keys()))
    ...
    def on_train_begin(self, logs={}):
        ...
        def manualstop(msg):
            ...
            type_list = ['MEMORY', 'UTILIZATION', 'ECC', 'TEMPERATURE', 'POWER', 'CLOCK', 'COMPUTE', 'PIDS',
            ...
            if msg['ToUserName'] == 'filehelper':
                ...
                if 'Stop at' in text:
                    ...
                    self.t_send('Command accepted,training will be stopped at epoch' + str(self.stopped_epoch),
                                toUserName='filehelper')
                # ==============================================================================
                #
                # ==============================================================================
                if any((k in text) for k in stop_training_cmdlist):
                    ...
                    self.t_send('Command accepted,stop training now at epoch' + str(self.epoch[-1] + 1),
                                toUserName='filehelper')
                # ==============================================================================
                #
                # ==============================================================================
                if any((k in text) for k in shut_down_cmdlist):
                    ...
                    if any((k in text) for k in [u'不保存模型', "don't save"]):
                        ...
                        save = True
                        filepath = (
                            self.GetMiddleStr(text, '[', ']') + '.h5' if self.GetMiddleStr(text, '[', ']') else (
                                                                                                                    self.fexten if self.fexten else self.validateTitle(
                                                                                                                        self.localtime)) + '.h5')
                    print('\n', filepath, '\n')
                ...
                if any((k in text) for k in get_fig_cmdlist):
                    ...
                    metrics = (
                        self.GetMiddleStr(text, '[', ']').split() if self.GetMiddleStr(text, '[', ']').split() else [
                            'all'])
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if self.savelog:
            sio.savemat((self.fexten if self.fexten else self.validateTitle(self.localtime)) + '_logs_batches' + '.mat',
                        {'log': np.array(self.logs_batches)})
            sio.savemat((self.fexten if self.fexten else self.validateTitle(self.localtime)) + '_logs_batches' + '.mat',
                        {'log': np.array(self.logs_epochs)})
        th.start_new_thread(self.get_fig, ())
        # ==============================================================================
        #         try:
